function [theta_1,theta_2] = inv_F(theta_1g,theta_2g,x,y,u,y_foot_F,x_foot_F, th)
...
while (epsilon1>= epsilon || epsilon2 >= epsilon && runs <= 10)
    ...
    delt_x = -r_1*c1 - r_2*c12 + x_foot_F - x - cos(th)*(body_length/2);
    delt_y = -r_1*s1 - r_2*s12 + y_foot_F - y - sin(th)*(body_length/2);
