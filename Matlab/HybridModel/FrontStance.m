function dydt = FrontStance(t,y,u,k_vals)
...
B_y = amp_y*sin((pi()/T_stance)*t);
B_x = -amp_x*sin((pi()/T_stance)*t);

hip_height = y(1) + 0.5*0.7*sin(y(5));
hip_height_dot = y(2) + y(6)*0.7*0.5;

if (t<0.2*T_stance)
    g_feedback = (1/(0.2*T_stance))*t;
elseif (t>0.8*T_stance)
    g_feedback = -(1/(0.2*T_stance))*t + 5;
else
    g_feedback = 1;
end
